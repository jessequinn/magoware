      ...
      sequelize.sync({force: (process.env.DB_SYNC_FORCE === 'true')})
        ...
        async.forEach(complete_menu_object, function (label, callback) {
          async.forEach(label.menu_list, function (menu_level_one, callback) {
            complete_api_group.push({
              id: menu_level_one.id,
              api_group_name: menu_level_one.api_group_name,
              description: menu_level_one.description
            });
            async.forEach(menu_level_one.api_list, function (api_url_obj, callback) {
              complete_api_url.push({
                api_url: api_url_obj.api_url,
                description: '????????',
                api_group_id: menu_level_one.id
              });
            }, function (error) {
        ...
        db.models['api_url'].bulkCreate(
          complete_api_url, {
            fields: ['api_url', 'description', 'api_group_id'],
            updateOnDuplicate: ['api_url', 'api_url_obj']
          }
        ).then(function (done) {
          return null;
        }).catch(function (err) {
